...
public:
	...
	struct BaseMetadata {
		BaseMetadata(int startFrame,
			...
			: startFrame(startFrame),
			  endFrame(endFrame),
			  initialDelay(sf::milliseconds(delay)),
			  inversionFrames(std::move(inversionFrames)),
	...
	enum class Priority { HIGH = 2, MEDIUM = 1, LOW = 0 };

	struct Metadata : public BaseMetadata {
		Metadata(int frameWidth,
				 int frameHeight,
				 int startFrame,
				 int endFrame,
				 int row,
