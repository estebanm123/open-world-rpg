...
public:
	...
	explicit ScoredInitializerPool(std::vector<std::unique_ptr<InitializerMetadata<Output, Arg>>> initializers)
...
protected:
	...
	virtual bool shouldInitialize(Arg arg) { return true; }
...
private:
	...
	void normalizeScoreDistribution() {
		...
		for (auto &metadata : initializers) {
			...
			metadata->score = sumToDistribute;	// should probably refactor assignment to another variable for clarity
												// [eg. score bound]
		}
