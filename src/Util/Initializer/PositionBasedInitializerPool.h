...
public:
	...
	PositionBasedInitializerPool(std::vector<std::unique_ptr<InitializerMetadata<T, Position>>> initializers,
								 int initializationThreshold = 400,
								 int maxInitializationLim = 500)
		: ScoredInitializerPool<T, Position>(std::move(initializers)),
		  initializationThreshold(initializationThreshold),
		  maxInitializationLim(maxInitializationLim) {}

	std::unique_ptr<T> initialize(Position pos) { return ScoredInitializerPool<T, Position>::initializeFrom(pos); }

protected:
	float computeScoreFrom(Position arg) override {
		auto size = this->initializers.size();
		auto hashVal = hash2ValuesModSize(
			arg.x,
			arg.y,
