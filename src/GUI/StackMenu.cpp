...
namespace gui {
    ...
    StackMenu::StackMenu(const sf::RenderWindow &window, float baseY)
            : basePosition((float) window.getSize().x / 2.0f, baseY), baseSize(300, 20) {
    ...
    StackMenu::StackMenu(const sf::Vector2f &position)
            : basePosition(position), baseSize(300, 20) {
    ...
    StackMenu::StackMenu(StackMenu &&other)
            : widgets(std::move(other.widgets)), background(std::move(other.background)),
              basePosition(other.basePosition), baseSize(other.baseSize) {
    }
