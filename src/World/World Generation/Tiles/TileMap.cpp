...
sf::Vector2f TileMap::convertLocalToGlobalCoords(sf::Vector2i localCoords, sf::Vector2f globalPos) {
	auto localCoordsFloat =
		static_cast<sf::Vector2f>(localCoords);	 // safe cast, as localCoords will never be very high
...
const sf::Vector2f &TileMap::getTopLeftPos() const { return pos; }

Tile *TileMap::getTile(int x, int y) { return tiles[x][y].get(); }
