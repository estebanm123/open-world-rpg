...
public:
	...
	ChunkEnvBasedEntityGenerator(int entityGenLimitMax = 100,
								 int entityGenLimitChange = 3,
								 int entityGenLimitInitial = 80);

protected:
	typedef std::array<std::array<bool, TileMap::SIZE_X>, TileMap::SIZE_Y> TilesSeen;

	virtual bool isEntityValid(Entity *entity,
							   const TileMap &tiles,
							   TilesSeen &tilesSeen,
...
private:
	...
	bool isEntityOverlappingOthersAndMarkAsSeen(Entity *entity,
												const sf::Vector2i &localCoords,
												TilesSeen &tilesSeen,
												const TileMap &tiles);
