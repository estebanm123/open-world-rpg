...
ActiveZone::ActiveZone(sf::Vector2f centerPos, sf::Vector2i size) : centerPos(centerPos), sizeInPx(
        {size.x * static_cast<float>(SpatialPartition::SLOT_WIDTH),
...
const sf::Vector2f &ActiveZone::getCenterPos() const {
    return centerPos;
}

const sf::Vector2f &ActiveZone::getSize() const {
    return sizeInPx;
}
