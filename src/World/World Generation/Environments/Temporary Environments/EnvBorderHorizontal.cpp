...
NeighboredEnv::Neighbors EnvBorderHorizontal::getCompatibleNeighbors(const sf::Vector2i &direction) const {
	if (direction == constants::NORTH) {
		return NeighboredEnv::Neighbors{std::make_unique<EnvBorderHorizontal>(nullptr, primaryEnv),
										primaryEnv,
	...
	if (direction == constants::SOUTH) {
		return NeighboredEnv::Neighbors{std::make_unique<EnvBorderHorizontal>(secondaryEnv, nullptr),
										secondaryEnv,
	...
	if (direction == constants::EAST) {
		return NeighboredEnv::Neighbors{std::make_unique<EnvBorderHorizontal>(*this),
...
std::string EnvBorderHorizontal::toString() const { return "H"; }
