...
struct EnvBorderCorner : public EnvBorderData {
	EnvBorderCorner(const std::shared_ptr<EnvWrapper> &primary,
	...
	const TileContainer *getTileContainer(const Env *primary,
										  const Env *secondary,
										  const sf::Vector2f &globalCoords) const override;
