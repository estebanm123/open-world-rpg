...
//
std::shared_ptr<NeighboredEnv> selectCandidate(const std::shared_ptr<EnvWrapper> &initialEnv,
											   NeighboredEnv::Neighbors &candidates) {
...
std::shared_ptr<NeighboredEnv> EnvAllocator::getEnvFromNeighbors(const std::shared_ptr<EnvWrapper> &initialEnv,
...
void allocateCorner(const sf::Vector2i &topLeft,
					const EnvAllocator::EnvMap &initialEnvs,
...
void EnvAllocator::allocateCorners(const EnvMap &initialEnvs, TmpNeighboredEnvs &finalEnvs) {
	...
	std::array<sf::Vector2i, 4> corners{sf::Vector2i{0, 0},
										sf::Vector2i{maxCornerTopLeftX, maxCornerTopLeftY},
										sf::Vector2i{0, maxCornerTopLeftY},
										sf::Vector2i{maxCornerTopLeftX, 0}};
...
void allocateColumn(const EnvAllocator::EnvMap &initialEnvs,
					EnvAllocator::TmpNeighboredEnvs &finalEnvs,
					sf::Vector2i bottom,
					sf::Vector2i top,
	...
	while (top.y <= bottom.y) {
		if (top == bottom) {
			finalEnvs[bottom.x][bottom.y] =
				EnvAllocator::getEnvFromNeighbors(initialEnvs[bottom.x][bottom.y], {bottom.x, bottom.y}, finalEnvs);
			if (isPriority) finalEnvs[bottom.x][bottom.y]->togglePriority();
		} else {
			finalEnvs[top.x][top.y] =
				EnvAllocator::getEnvFromNeighbors(initialEnvs[top.x][top.y], {top.x, top.y}, finalEnvs);
...
void allocateRow(const EnvAllocator::EnvMap &initialEnvs,
				 EnvAllocator::TmpNeighboredEnvs &finalEnvs,
				 sf::Vector2i right,
				 sf::Vector2i left,
				 bool isPriority = false) {
	...
	while (left.x <= right.x) {
		if (left == right) {
			finalEnvs[right.x][right.y] =
				EnvAllocator::getEnvFromNeighbors(initialEnvs[right.x][right.y], {right.x, right.y}, finalEnvs);
			if (isPriority) finalEnvs[right.x][right.y]->togglePriority();
		} else {
			finalEnvs[left.x][left.y] =
				EnvAllocator::getEnvFromNeighbors(initialEnvs[left.x][left.y], {left.x, left.y}, finalEnvs);
...
void EnvAllocator::allocateSharedSides(const EnvAllocator::EnvMap &initialEnvs,
									   EnvAllocator::TmpNeighboredEnvs &finalEnvs) {
	const int maxCornerTopLeftX =
		static_cast<int>(finalEnvs.size() - 2);	 // make the class stateful and store this in ctor?
	const int maxCornerTopLeftY = static_cast<int>(finalEnvs[0].size() - 2);
...
void EnvAllocator::allocateInnerCells(const EnvAllocator::EnvMap &initialEnvs,
									  EnvAllocator::TmpNeighboredEnvs &finalEnvs) {
