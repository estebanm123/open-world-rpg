...
std::string EnvWrapper::toString() const { return "E"; }

bool EnvWrapper::fillWildcardIfExists(const std::shared_ptr<EnvWrapper> &filler) { return false; }
...
const NeighboredEnv::TileContainerWrapper EnvWrapper::extractTileMetadata(const sf::Vector2f &globalCoords) const {
	...
	Tile::Metadata metadata{std::make_unique<const EnvProxy>(env), globalCoords, env->getSpriteSheetPath()};
	return {std::move(metadata), tileContainer};
}

const Env *EnvWrapper::getEnv() const { return env; }

std::pair<const Env *, const Env *> EnvWrapper::getEnvs() const { return std::make_pair(env, nullptr); }
