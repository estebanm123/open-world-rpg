...
float EnvBorderData::getExtraRotationAngle() const { return 0; }
...
const NeighboredEnv::TileContainerWrapper EnvBorderData::extractTileMetadata(const sf::Vector2f &globalCoords) const {
	...
	Tile::Metadata metadata{std::make_unique<const EnvBorder>(),
							globalCoords,
							spriteSheetPath,
...
const TileContainer *EnvBorderData::getTileContainer(const Env *primary,
													 const Env *secondary,
