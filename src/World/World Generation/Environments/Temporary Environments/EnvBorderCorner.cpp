...
EnvBorderCorner::EnvBorderCorner(const std::shared_ptr<EnvWrapper> &primary,
								 ...
								 float rotationAngle)
	: EnvBorderData(primary, secondary),
...
NeighboredEnv::Neighbors EnvBorderCorner::getNorthNeighbors() const {
	...
	if (rotationAngle == 90) {
		return NeighboredEnv::Neighbors{std::make_unique<EnvBorderCorner>(primaryEnv, secondaryEnv, 180),
...
NeighboredEnv::Neighbors EnvBorderCorner::getSouthNeighbors() const {
	...
	if (rotationAngle == 270) {
		return NeighboredEnv::Neighbors{std::make_unique<EnvBorderCorner>(primaryEnv, secondaryEnv, 0),
...
std::string EnvBorderCorner::toString() const { return "C" + std::to_string(static_cast<int>(rotationAngle)); }

float EnvBorderCorner::getExtraRotationAngle() const { return rotationAngle; }

const TileContainer *EnvBorderCorner::getTileContainer(const Env *primary,
													   const Env *secondary,
													   const sf::Vector2f &globalCoords) const {
	return primary->getCornerTileContainer(secondary, globalCoords);
}
