...
std::shared_ptr<SingleTileContainer> makeSingleTileContainer(int index) {
	return std::make_shared<SingleTileContainer>(
		SingleTileContainer({index * TILE_SIZE_INT_X, 0, TILE_SIZE_INT_X, TILE_SIZE_INT_Y}));
}
...
TileContainer::TileContainers initializeCompleteAnimatedTiles(int startTile,
															  int numTiles,
															  int numFramesPerTile,
															  sf::Time delay) {
	TileContainer::TileContainers tileContainers;
...
void EnvManager::initializeBorderTileContainers(Env *env) {
	...
	for (auto &borderData : config->borderDataCollection) {
		...
		Animation::BaseMetadata *animMetadata = config->animMetadata		   ? config->animMetadata.get()
												: otherEnvConfig->animMetadata ? otherEnvConfig->animMetadata.get()
																			   : nullptr;
...
const Env *EnvManager::getEnv(Env::EnvId envId) { return envs[envId].get(); }
