...
inline void initialize() {
	EnvManager::createEnv(std::make_unique<Env::Config>(WATER,
														"Tiles/Water",
														1,
														std::vector<EnvNeighborInfo>{EnvNeighborInfo{DIRT, 0, 2}},
	...
	EnvManager::createEnv(std::make_unique<Env::Config>(
		...
		"Tiles/Dirt",
		6,
		std::vector<EnvNeighborInfo>{EnvNeighborInfo{WATER, 1, 3}, EnvNeighborInfo{SAND, 0, 4}},
		std::make_unique<DirtPropFactory>(),
		std::make_unique<DirtBeastFactory>()));

	EnvManager::createEnv(std::make_unique<Env::Config>(SAND,
														"Tiles/Sand",
														4,
