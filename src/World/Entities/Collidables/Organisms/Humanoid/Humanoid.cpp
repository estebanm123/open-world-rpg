...
Humanoid::Humanoid(const sf::Vector2f &globalPosition,
...
EntitySprite &Humanoid::getSprite() { return sprite; }
...
std::vector<std::unique_ptr<EntitySprite>> Humanoid::initializeSprites(const sf::Vector2f &pos,
																	   const std::string &spriteSheetBody,
																	   const std::string &spriteSheetHead) {
	using namespace animConstants;
	std::vector<std::unique_ptr<EntitySprite>> sprites;
	sprites.push_back(std::make_unique<ShadowedSpriteReg>(
		ShadowedSpriteReg{spriteSheetBody,
						  pos,
						  {BASE_FRAME_WIDTH / 2.f, BASE_FRAME_HEIGHT / 3.f},
						  std::move(HumanoidAnimInitializer::initializeEmptyBody())}));
	sprites.push_back(
		std::make_unique<SpriteReg>(SpriteReg::Config{spriteSheetHead,
													  pos,
													  {BASE_FRAME_WIDTH / 2.f, BASE_FRAME_HEIGHT / 1.8f},
													  std::move(HumanoidAnimInitializer::initializeEmptyHead()),
													  {0, 0, BASE_FRAME_WIDTH, BASE_FRAME_HEIGHT}}));
	return sprites;
}

void Humanoid::accept(EntityVisitor *visitor) { visitor->visit(this); }
...
void Humanoid::attemptPickup() { this->setCurrentAction(&HumanoidActions::Touch); }

void Humanoid::initializeFootprintGenerator() {
	auto pos = getPosition();
	auto size = getSize();
	auto spriteConfig = SpriteReg::CopyableConfig{
		.spriteSheet = "Npc/Footprints/Humanoid",
		.pos = pos,
		.origin = sf::Vector2f{static_cast<float>(size.x / 2), static_cast<float>(size.y / 2)},
		.defaultFrame = sf::IntRect{0, 0, static_cast<int>(size.x), static_cast<int>(size.y)}};
	FootprintGenerator footprintGenerator{spriteConfig, size.y * 3, {size.x / 2.3f, size.y / 2.3f}, pos};
	setSurfaceEffectGenerator(std::make_unique<FootprintGenerator>(footprintGenerator));
}

bool Humanoid::isAttemptingPickUp() const { return isPickingUp; }

SingleHitbox *Humanoid::getPickUpHitbox() { return pickupZone; }

void Humanoid::setIsAttemptingPickUp(bool isAttemptingPickUp) { isPickingUp = isAttemptingPickUp; }
